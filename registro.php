...
if(isset($_REQUEST['btn_register'])) //compruebe el nombre del botón "btn_register" y configúrelo
{
	...
	{	
		try
		{	
			$select_stmt=$db->prepare("SELECT username, email FROM logi 
			...
			{
				$insert_stmt=$db->prepare("INSERT INTO logi(username,email,password,role) VALUES(:uname,:uemail,:upassword,:urole)"); //Consulta sql de insertar			
